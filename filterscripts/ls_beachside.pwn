...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}按下 '{FFFFFF}N{CCCCCC}' 使用电梯", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0, 0, 1);
				...
				if (i <= 12)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
							...
							GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~h~LS BeachSide Elevator Is~n~~y~~h~Already On This Floor...~n~~w~Walk Inside It~n~~w~And Press 'N'", 3500, 3);

							// Display chat text message to the player
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* 电梯已经在这个楼层了... 往里面走,然后按下 '{FFFFFF}N{CCCCCC}'");
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* 已经按下电梯按钮了... 它目前正在前往 %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* 已经按下电梯按钮了... 它目前在 %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}按下 '{FFFFFF}N{CCCCCC}' 使用电梯", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}按下 '{FFFFFF}N{CCCCCC}' 呼叫", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "电梯...", string, "选择", "返回");
